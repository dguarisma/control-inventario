...
if (empty($_SESSION['username']) && empty($_SESSION['password'])){
    ...
    if ($_GET['act']=='insert') {
        if (isset($_POST['Guardar'])) {
            ...
            $query = mysqli_query($mysqli, "INSERT INTO transaccion_inventory(codigo_transaccion,fecha,codigo,numero,created_user,tipo_transaccion) 
            ...
            if ($query) {
                ...
                if ($query1) {                       
                    ...
                    header("location: ../../main.php?module=inventory_transaction&alert=1");
                }
            }   
        }   
    }
    ...
    elseif ($_GET['act']=='delete') {
        if (isset($_GET['id'])) {
            ...
            $query = mysqli_query($mysqli, "DELETE FROM transaccion_inventory WHERE codigo_transaccion='$codigo_transaccion'") or die('error '.mysqli_error($mysqli));
            
            if ($query) { header("location: ../../main.php?module=inventory_transaction&alert=3");
            }
        }
    }    
}       
